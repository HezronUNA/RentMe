...
export function DestinationSection({ variant, ubicacion, onUbicacionChange }: DestinationSectionProps) {
  ...
  return (
    <div className="px-4 py-4 border-b border-zinc-200/80 relative" ref={dropdownRef}>
      <div className="text-zinc-700 text-xs font-semibold tracking-wide mb-2 uppercase">Ubicación</div>
      <div className="relative">
        <button
          onClick={handleToggleDropdown}
        ...
        >
          ...
          <div className="text-gray-800 text-sm font-medium flex-1 transition-colors">
            {isLoading ? 'Cargando ubicaciones...' : ubicacion || 'Selecciona una ubicación'}
          </div>
          {!isLoading && (
